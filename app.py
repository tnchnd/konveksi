...
@app.route('/set_shipping_option', methods=['POST'])
def set_shipping_option():
    if request.method == 'POST':
        order_item_id = request.form['order_item_id']
        shipping_option = request.form['shipping_option']
        db.pesanan.update_one({'_id': ObjectId(order_item_id)}, {'$set': {'via_pengiriman': shipping_option}})
    return redirect(url_for('keranjang'))

@app.route('/no_resi', methods=['POST'])
def set_no_resi():
    if request.method == 'POST':
        order_item_id = request.form['order_item_id']
        no_resi = request.form['no_resi']
        db.pesanan.update_one({'_id': ObjectId(order_item_id)}, {'$set': {'nomor_resi': no_resi}})
    return redirect(url_for('keranjang'))
